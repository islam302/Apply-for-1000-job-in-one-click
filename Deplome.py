    ...
    def main(self, number=0):
        ...
        for username, password in users.items():
            ...
            for subject in subjects:
                ...
                # Handling lecture links
                lecture_links = self.get_units_links(subject_href)
                for link in lecture_links:
                    print(f"Lecture: {link['label']}, URL: {link['link']}")

                    # Checking video
                    video_watched = self.watch_video(link['link'])
                    if video_watched:
                        print("Video watched successfully.")
                    else:
                        print("Error watching video.")

                    # Checking audio
                    audio_listened = self.listen_audio(link['link'])
                    if audio_listened:
